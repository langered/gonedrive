...
var _ = Describe("File-Service", func() {
	...
	"name": "folder1_2"
		...
		It("uploads a local file under the given path", func() {
			...
			success, err := client.Upload(mockHttpClient, "abc123", "folder1/folder1_2/test.txt", "test-content")
		...
		It("uploads a local file under the root level", func() {
			...
			success, err := client.Upload(mockHttpClient, "abc123", "test.txt", "test-content")
